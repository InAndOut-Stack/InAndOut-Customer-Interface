...
import { Box, Typography, Stack, Grid, TextField, Button, Card, CardContent, Divider, CircularProgress } from '@mui/material';
...
export default function StandSelectionPage() {
    ...
    const filteredStands = stands.filter(stand => 
        (stand.name || `Article ${stand.articleId}`).toLowerCase().includes(searchQuery.toLowerCase())
    );
    ...
    return (
        <Stack sx={{ 
            ...
            <Typography sx={{ color: colors.purpleColor, fontSize: '40px', fontFamily: '"Creato", sans-serif', fontWeight: 700, mb: 4, textAlign: 'center' }}>
                Select Articles
            </Typography>

            <Grid container spacing={4} sx={{ flexGrow: 1, backgroundColor: 'rgba(255, 255, 255, 0.4)', borderRadius: '30px', p: 3, backdropFilter: 'blur(10px)' }}>
                {/* Left Panel: Shopping List */}
                <Grid size={{ xs: 12, md: 4}}>
                    <Card sx={{ height: '100%', borderRadius: '24px', boxShadow: 'none', border: `2px solid ${colors.orangeColor}`, display: 'flex', flexDirection: 'column' }}>
                        <CardContent sx={{ flexGrow: 1, p: 4 }}>
                            <Typography variant="h5" sx={{ fontFamily: '"Creato", sans-serif', fontWeight: 600, color: colors.orangeColor, mb: 2 }}>
                            ...
                            <Stack spacing={2} sx={{ maxHeight: '50vh', overflowY: 'auto' }}>
                                ...
                                {shoppingCart.map((item) => (
                                    <Box key={item.standId} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <Typography sx={{ fontFamily: '"Creato", sans-serif', fontWeight: 500 }}>
                                            {item.name || `Article ${item.articleId}`}
                                        </Typography>
                                        <Button size="small" color="error" onClick={() => handleRemoveFromCart(item.standId)}>
                                            Remove
                                        </Button>
                                    </Box>
                                ))}
                        ...
                        <Box sx={{ p: 3 }}>
                            ...
                            >
                                {isGenerating ? <CircularProgress size={24} color="inherit" /> : 'Generate Route'}
                            </Button>
                        </Box>
                    </Card>
                </Grid>
                ...
                <Grid size={{ xs: 12, md: 8 }}>
                    <Stack spacing={3}>
                        ...
                        {isLoading ? (
                            ...
                            <Grid container spacing={2} sx={{ maxHeight: '65vh', overflowY: 'auto', p: 1 }}>
                                {filteredStands.map(stand => (
                                    <Grid size={{ xs: 12, sm: 6, lg: 4 }} key={stand.standId}>
                                        <Card sx={{ borderRadius: '16px', border: `1px solid #ddd`, boxShadow: 'none' }}>
                                            <CardContent>
                                                <Typography sx={{ fontFamily: '"Creato", sans-serif', fontWeight: 600, mb: 1 }}>
